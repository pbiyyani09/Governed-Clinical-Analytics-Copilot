...
def execute_query(
    ...
    with get_connection(db_path, resolved_timeout, resolved_max_rows) as conn:
        ...
        # join — exactly what a hallucinated SQL query produces on the 668k-row
        # chartevents table). conn.interrupt() IS checked inside those loops, so a
        # watchdog thread is the reliable hard wall-clock stop.
